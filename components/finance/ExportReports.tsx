...
export default function ExportReports() {
  const generateReport = async (months: number) => {
    ...
    // Revenue
    const totalRevenueByWorker = relevantAppointments
      .filter(apt => apt.worker === 'dina')
      .reduce((sum, apt) => sum + apt.price, 0);

    // Expenses
    const totalExpensesByWorker = relevantExpenses
    ...
    return {
      ...
      totalExpenses,
      netProfit,
      totalRevenueByWorker,
      totalExpensesByWorker,
  ...
  const exportPDF = async (type: 'month' | '6months' | '12months') => {
    ...
    try {
      ...
      row('Total Revenue', `$${report.totalRevenueByWorker.toFixed(2)}`);

      y += 6;

      /* ===== Expenses ===== */
      sectionTitle('Expenses');
      row('Total Expenses', `$${report.totalExpensesByWorker.toFixed(2)}`);
