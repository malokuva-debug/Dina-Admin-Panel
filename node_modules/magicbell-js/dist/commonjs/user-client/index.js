"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Client = exports.Environment = void 0;
const tslib_1 = require("tslib");
const index_js_1 = require("./services/channels/index.js");
const index_js_2 = require("./services/integrations/index.js");
const index_js_3 = require("./services/notifications/index.js");
var environment_js_1 = require("./http/environment.js");
Object.defineProperty(exports, "Environment", { enumerable: true, get: function () { return environment_js_1.Environment; } });
tslib_1.__exportStar(require("./http/index.js"), exports);
tslib_1.__exportStar(require("./services/channels/index.js"), exports);
tslib_1.__exportStar(require("./services/common/index.js"), exports);
tslib_1.__exportStar(require("./services/integrations/index.js"), exports);
tslib_1.__exportStar(require("./services/notifications/index.js"), exports);
class Client {
    constructor(config) {
        Object.defineProperty(this, "config", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: config
        });
        Object.defineProperty(this, "channels", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "integrations", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "notifications", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        this.channels = new index_js_1.ChannelsService(this.config);
        this.integrations = new index_js_2.IntegrationsService(this.config);
        this.notifications = new index_js_3.NotificationsService(this.config);
    }
    set baseUrl(baseUrl) {
        this.channels.baseUrl = baseUrl;
        this.integrations.baseUrl = baseUrl;
        this.notifications.baseUrl = baseUrl;
    }
    set environment(environment) {
        this.channels.baseUrl = environment;
        this.integrations.baseUrl = environment;
        this.notifications.baseUrl = environment;
    }
    set timeoutMs(timeoutMs) {
        this.channels.timeoutMs = timeoutMs;
        this.integrations.timeoutMs = timeoutMs;
        this.notifications.timeoutMs = timeoutMs;
    }
    set token(token) {
        this.channels.token = token;
        this.integrations.token = token;
        this.notifications.token = token;
    }
}
exports.Client = Client;
// c029837e0e474b76bc487506e8799df5e3335891efe4fb02bda7a1441840310c
//# sourceMappingURL=index.js.map